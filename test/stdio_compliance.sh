...
echo ""
echo "   # Then run:"
echo "   mcp-validate --transport stdio --env THREDS_DB_PATH=:memory: \\"
echo "     --json-report compliance-report.json --debug -- ./bin/threds-mcp"
echo ""
echo "3. INTERACTIVE TESTING (For manual debugging)"
echo "   npx @modelcontextprotocol/inspector ./bin/threds-mcp"
...
echo "     \"mcpServers\": {"
echo "       \"threds\": {"
echo "         \"command\": \"./bin/threds-mcp\","
echo "         \"env\": { \"THREDS_DB_PATH\": \":memory:\" }"
echo "       }"
echo "     }"
echo "   }"
echo "   EOF"
echo ""
echo "   npx @wong2/mcp-cli --config test-config.json call-tool threds:list_records"
echo ""
echo "═══════════════════════════════════════════════════════════"
echo ""
